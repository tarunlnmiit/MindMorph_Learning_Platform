...
DECISION:
- LOW score (struggled, < 40): add 1-2 REMEDIAL sub-skill nodes that break down the specific thing
  they missed. Each remedial node's edge points INTO the graded node (source = remedial node,
  target = graded node) — they are new prerequisites. Set remediation_focus to the concrete gaps
  (e.g. "list comprehensions", "off-by-one in loops").
- HIGH score (mastered, >= 80): add NO new nodes. Add unlock edges from the graded node to sensible
  downstream nodes already in the graph (source = graded node, target = downstream), only if such an
  edge does not already exist. Leave remediation_focus empty.
- MIDDLE score (40-79): NO structural change — return empty new_nodes and empty new_edges. If useful,
